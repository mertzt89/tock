...
// Size of the metadata to store ShortId->region mappings
...
struct NonvolatileStorageAssignedRegion {
    /// The application that owns the region.
    short_id: ShortId,
    /// The index of the region.
    index: usize,
}

pub struct NonvolatileStorage<'a, const APP_STORAGE_SIZE: usize, const NUM_APPS: usize> {
    ...
    cached_assignments: MapCell<[NonvolatileStorageAssignedRegion]>,
}
...
impl<'a> NonvolatileStorage<'a> {
    ...
    /// Return the assigned index if in the cache, otherwise return `None`.
    fn get_cached_assignment(&self, short_id: ShortId) -> Option<usize> {
        self.cached_assignments.map(|assignments| {
            for assignment in assignments {
                if assignment.short_id == short_id {
    ...
    fn enqueue_command(
        ...
        if offset >= APP_STORAGE_SIZE
            || length > APP_STORAGE_SIZE
            || offset + length > APP_STORAGE_SIZE
        {
...
impl<const APP_STORAGE_SIZE: usize, const NUM_APPS: usize> SyscallDriver
    for NonvolatileStorage<'_>
{
    /// Command interface.
    ///
    ...
    fn command(
        ...
        match command_num {
            ...
            1 => {
                ...
                CommandReturn::success_u32(APP_STORAGE_SIZE as u32)
            }
